...
export default {
    ...
    update(tower, dt) {
        if (tower.stormOfArrows && tower.stormOfArrows.active) {
            ...
            if (tower.stormOfArrows.timer <= 0) {
                tower.stormOfArrows.active = false;
            } else {
                const soa = tower.stormOfArrows;
                const nearby = GameEngine.enemyGrid.query(soa.x, soa.y, soa.radius);
                for (let e of nearby) {
                    if (!e.alive) continue;
                    if (Utils.distance(soa.x, soa.y, e.x, e.y) < soa.radius) {
                        if (e.stormHitTimer > 0) continue;
                        if (Math.random() < soa.chance) {
                            let dmgType = { isSharp: true, canHitLead: true };
                            let dmg = soa.dmg;
    ...
    draw(ctx, tower, isPreview) {
        if (!isPreview && tower.stormOfArrows && tower.stormOfArrows.active) {
            ...
            ctx.beginPath();
            ctx.arc(soa.x, soa.y, soa.radius, 0, Math.PI * 2);
            ctx.fill();
            ctx.globalAlpha = 1;
            ...
            for(let i=0; i<15; i++) {
                ...
                let r = Math.random() * soa.radius;
                let x1 = soa.x + Math.cos(angle) * r;
                let y1 = soa.y + Math.sin(angle) * r;
                ctx.beginPath();
                ctx.moveTo(x1, y1 - 12);
                ctx.lineTo(x1, y1);
                ctx.stroke();
    ...
    fire(tower, target, damage, dmgType, isCrit, effects) {
        ...
        for(let i=0; i<count; i++) {
            let p = new Projectile(tower.x, tower.y, damage, target, 'arrow', tower.stats.projectileSpeed, tower.stats.pierce, tower.stats.lifespan, null, null, 5 * (i - (count-1)/2), tower, dmgType);
            p.isCrit = isCrit;
            if (isExplosive) {
                p.effects = p.effects || {};
                p.effects.isExplosive = true;
                p.effects.explosionRadius = 80;
                p.effects.explosionDamage = 1;
                p.effects.explosionPierce = 10;
            }
